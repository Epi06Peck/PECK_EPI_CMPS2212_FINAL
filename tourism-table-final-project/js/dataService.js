...
export function createDataService(eventBus, dataUrl) {
  ...
  function applySearch(rows, searchTerm) {
    ...
    return rows.filter((row) =>
      row.country.toLowerCase().includes(searchTerm.trim().toLowerCase()),
    );
  }
  ...
  function applySort(rows, sortColumn, sortDirection) {
    ...
    clonedRows.sort((a, b) => {
      ...
      if (sortColumn === "month") {
        aValue = MONTH_ORDER[aValue];
        bValue = MONTH_ORDER[bValue];
      }
  ...
  function selectRow(id) {
    ...
    const row = state.allRows.find((row) => row.id === numID);
    if (!row) {
      return;
    }
